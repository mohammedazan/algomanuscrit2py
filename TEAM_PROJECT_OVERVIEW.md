...
```
# Important note
⚠️ المشكل اللي وقعنا فيه
النموذج كيخرج نتائج سيئة حتى على صور من training set.
هذا معناه أن المشكل ماشي overfitting، ولكن مشكل بنيوي.
السبب:
- كان عندنا عدد كبير جداً من الحروف (≈120+) في الـ dataset:
- حروف كبيرة وصغيرة
- accents
- رموز مختلفة
- تنوع غير موحد في الكتابة
وبما أن dataset صغير (152 صورة فقط)،
النموذج ما قدرش يتعلم هذا الـ search space الكبير ديال CTC.
🎯 الخلاصة التقنية
كلما كان عدد الحروف أكبر:
- Output layer تكبر
- احتمالات prediction تكبر
- التدريب يصعب
- النتائج تضعف
- Dataset صغير + Vocabulary كبير = نتائج ضعيفة جداً
...
2️⃣ Vocabulary Reduction
نحتفظ فقط بالحروف والرموز الضرورية للخوارزميات:
- a-z
- 0-9
- () + - * / = < > :
- space و newline
(تقريباً 50–60 حرف فقط بدل 120)

3️⃣ إعادة Training
إعادة بناء charset
إعادة تدريب النموذج
تقييم النتائج من جديد

🏁 الهدف
تقليل مساحة البحث (Search Space)
وتسهيل مهمة CTC
باش النموذج يقدر يتعلم فعلاً من dataset صغير.
